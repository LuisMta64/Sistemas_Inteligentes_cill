...
class Genetic_Algorithm():
    ...
    def init_population(self):
        ...
        self.fitness = np.array([ff.fitnessInference(self.chromosomes[i], self.n_genes, self.n_alleles, self.scale, self.offset) 
                    for i in range(self.size_population)])
    ...
    def start(self):
        ...
        while generation_counter <= self.generations:
            ...
            offspring_fitness = np.array(([ff.fitnessInference(offspring_mutated[i], self.n_genes, self.n_alleles, self.scale, self.offset) for i in range (2)]))
